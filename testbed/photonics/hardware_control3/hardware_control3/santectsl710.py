...
class SantecTSL710(object):

    def __init__(self, address='GPIB0::29'):
    ...
    def set_wavelength(self, wav):
        ...
        # set lambda
        self.instr.write('SOUR:WAV ' + str(wav))
        time.sleep(delay)
    ...
    def set_power(self, val, power_unit='dBm'):
        ...
        # set power
        self.instr.write('SOUR:POW ' + str(val))
    ...
    def wavelength_sweep(self, wl_start, wl_stop, pwr, mode, speed, delay=0, step=0, runFxn = runDefault):
        ...
        if (wl_start < WL_MIN) or (wl_start > WL_MAX):
            ...
            self.instr.write('SOUR:WAV:SWE:STAR ' + str(wl_start))
        ...
        if (wl_stop < WL_MIN) or (wl_stop > WL_MAX):
            ...
            self.instr.write('SOUR:WAV:SWE:STOP ' + str(wl_stop))
        ...
        if mode not in [0, 1, 2, 3]:
            ...
            self.instr.write('SOUR:WAV:SWE:MOD ' + str(mode))
        if mode in [0, 2]: #get step if needed
            if (step < WL_STEP_MIN) or (step > WL_STEP_MAX):
                ...
                step = round(step, 4)
                self.instr.write('SOUR:WAV:SWE:STEP ' + str(step))
        ...
        if (speed < SWEEP_SPEED_MIN) or (speed > SWEEP_SPEED_MAX):
            ...
            self.instr.write('SOUR:WAV:SWE:SPE ' + str(speed))
...
class SantecLaser(SantecTSL710):
    pass

if __name__ == '__main__':
    obj = SantecTSL710('GPIB0::29')
    obj.setup_basic()
